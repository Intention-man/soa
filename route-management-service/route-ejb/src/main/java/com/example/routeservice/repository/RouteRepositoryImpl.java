...
public class RouteRepositoryImpl implements RouteRepository {
    ...
    private Predicate buildComparisonPredicate(CriteriaBuilder cb, Path<?> path,
        ...
        if (parsed instanceof Number num) {
            Class<? extends Number> numberClass = (Class<? extends Number>) num.getClass();
            jakarta.persistence.criteria.Expression<? extends Number> specificPath = path.as(numberClass);

            return switch (operation) {
                case "gt" -> cb.gt(specificPath, num);
                case "gte", "min" -> cb.ge(specificPath, num);
                case "lt" -> cb.lt(specificPath, num);
                case "lte", "max" -> cb.le(specificPath, num);
                default -> null;
            };
